...
def consultar_ordem_carga():
    ...
    SELECT
        ...
        i.descrprod,
        i.referencia,
        i.AD_REFERENCIA2 AS ad_referencia2,
    ...
    try:
        ...
        for row in resultados:
            lista_itens.append({
                ...
                "descrProd": row[13],
                "referencia": row[14],
                "referencia2": row[15] if row[15] is not None else None,
                "validaCodBarra": row[16],
                "codVol": row[17],
                "marca": row[18],
                "qtdEmb": row[19].strip() if row[19] else "",
                "numeroNota": row[20],
                "qtdNeg": float(row[21]) if row[21] is not None else 0.0,
                "qtdVol": float(row[22]) if row[22] is not None else 0.0,
                "vlrTot": float(row[23]) if row[23] is not None else 0.0,
                "descrOper": row[24],
                "doca": row[25],
                "horaSaida": row[26].strftime('%Y-%m-%d %H:%M:%S') if row[26] else None,
                "seqCarga": row[27]
...
def cobranca_cidades():
    sql = "SELECT CODCID, UF, NOMECID FROM TSICID ORDER BY NOMECID"
    ...
    try:
        ...
        dados = [{"codCid": row[0], "uf": row[1], "nomeCid": row[2]} for row in resultados]
...
def cobranca_vendedores():
    sql = "SELECT CODVEND, APELIDO FROM TGFVEN ORDER BY APELIDO"
    ...
    try:
        ...
        dados = [{"codVend": row[0], "apelido": row[1]} for row in resultados]
...
def cobranca_parceiros():
    sql = "SELECT CODPARC, NOMEPARC, RAZAOSOCIAL, CGC_CPF FROM TGFPAR ORDER BY NOMEPARC"
    ...
    try:
        ...
        dados = [{"codParc": row[0], "nomeParc": row[1], "razaoSocial": row[2], "cgcCpf": row[3]} for row in resultados]
...
@app.route("/api/receitas-vencidas", methods=["POST"])
def receitas_vencidas():
    ...
    if dt_inicial and dt_final:
        filtros_extras.append("AND FIN.DTVENC BETWEEN TO_DATE(:DT_INICIAL, 'YYYY-MM-DD') AND TO_DATE(:DT_FINAL, 'YYYY-MM-DD')")
