...
Uma linha por vendedor, sem filtro (são poucas dezenas). `codVend: 0` é a linha "SEM VENDEDOR": título sem vendedor no financeiro, sem vendedor interno direto pela nota e sem vendedor efetivo único recuperável da renegociação — dívida real, só sem dono. Se o `AD_CODVENDINT` resolvido apontar para um código que não existe na `TGFVEN`, a linha sai com esse `codVend` e apelido "SEM VENDEDOR" — é cadastro a corrigir no Sankhya.
